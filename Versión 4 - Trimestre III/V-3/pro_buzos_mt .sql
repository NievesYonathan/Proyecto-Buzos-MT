...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 21-08-2024 a las 14:09:49
...
-- Funciones
--
CREATE DEFINER=`root`@`localhost` FUNCTION `obtener_fecha_asignacion` (`id` INT) RETURNS DATE DETERMINISTIC BEGIN
    DECLARE fecha DATE;
    
    -- Inicializamos la variable fecha con el valor de la consulta
    SELECT emp_tar_fecha_asignacion INTO fecha
    FROM emp_tarea
...
CREATE TABLE `materia_prima` (
  ...
  `imagen_mat_prima` varchar(30) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
...
INSERT INTO `materia_prima` (`id_materia_prima`, `mat_pri_nombre`, `mat_pri_descripcion`, `mat_pri_unidad_medida`, `mat_pri_cantidad`, `mat_pri_estado`, `fecha_compra_mp`, `proveedores_id_proveedores`, `imagen_mat_prima`) VALUES
(11, 'Tela Algodón', 'Tela de algodón para buzos', 'metros', 500, 'Existente', '2024-01-01', 10105885, NULL),
(12, 'Hilo de Poliester', 'Hilo resistente para costura', 'rollos', 200, 'Existente', '2024-01-05', 10274586, NULL),
(13, 'Cremalleras', 'Cremalleras de 30 cm', 'unidades', 1000, 'Existente', '2024-01-10', 10378547, NULL),
(14, 'Botones', 'Botones de plástico', 'unidades', 5000, 'Existente', '2024-01-15', 10452456, NULL),
(15, 'Elástico', 'Elástico de 2 cm', 'metros', 300, 'Agotado', '2024-01-20', 10547856, NULL),
(16, 'Tela Lana', 'Tela de lana para buzos', 'metros', 150, 'Existente', '2024-01-25', 10652867, NULL),
(17, 'Hilo de Algodón', 'Hilo suave para costura', 'rollos', 250, 'Existente', '2024-02-01', 10769878, NULL),
(18, 'Cierres', 'Cierres de 50 cm', 'unidades', 800, 'Deshabilitado', '2024-02-05', 14578608, NULL),
(19, 'Broches', 'Broches metálicos', 'unidades', 1500, 'Existente', '2024-02-10', 10652867, NULL),
(20, 'Tela Sintética', 'Tela sintética para buzos', 'metros', 400, 'Existente', '2024-02-15', 10652867, NULL);
...
CREATE TABLE `perfil` (
  ...
  `per_clave` varbinary(50) NOT NULL COMMENT 'Atributo que identifica la clave del perfil',
...
(1, 'admin', 0xc806d6eab39db432b90fa889d066a1f1, '2024-01-01'),
(2, 'jefe_inv', 0xee5a75707f1c61f18d92064c1ce27f18, '2024-02-01'),
(3, 'jefe_prod', 0x624d4459e576408ce2580ca05a5a0bc2, '2024-03-01'),
(4, 'empleado1', 0x99dca148e3b00a710c6e8f0a8f61efc5, '2024-04-01'),
(5, 'proveedor1', 0xe3d6eb04bc8735d75cacce7c3c1fd070, '2024-05-01'),
(6, 'cliente1', 0x5616ca03f08589ba23fb72f2b54693d9, '2024-06-01'),
(7, 'gerente', 0x53cd749f7135a57f52ed259c09fe9842, '2024-07-01'),
(8, 'supervisor', 0x88714d0d8d1a6bd91724fe8786c11308, '2024-08-01'),
(9, 'contador', 0x5c0d4bba7d232ce63630cb88030e5ec9, '2024-09-01'),
(10, 'auditor', 0xd9fc67ac019310ddb76ae5010f4887e9, '2024-10-01');

--
-- Disparadores `perfil`
--
DELIMITER $$
CREATE TRIGGER `before_insert_Usuarios` BEFORE INSERT ON `perfil` FOR EACH ROW BEGIN
    -- Encriptar el campo Clave antes de insertar
    SET NEW.per_clave = AES_ENCRYPT(NEW.per_clave, 'BUZOSMT');
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_update_Usuarios` BEFORE UPDATE ON `perfil` FOR EACH ROW BEGIN
    -- Encriptar el campo Clave antes de actualizar
    SET NEW.per_clave = AES_ENCRYPT(NEW.per_clave, 'BUZOSMT');
END
$$
DELIMITER ;
...
--
-- Volcado de datos para la tabla `reg_mat_prima`
--
...
CREATE TABLE `reg_pro_fabricados` (
  ...
  `imagen_producto_F` varchar(30) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
...
INSERT INTO `reg_pro_fabricados` (`id_reg_prod_fabricados`, `reg_pf_cantidad`, `reg_pf_fecha_registro`, `reg_pf_talla`, `reg_pf_color`, `reg_pf_material`, `reg_pf_tipo_prenda`, `produccion_id_produccion`, `imagen_producto_F`) VALUES
(1, 100, '2024-03-01', 'M', 'Rojo', 'Algodón', 'Buzo', 1, NULL),
(2, 200, '2024-03-05', 'L', 'Azul', 'Poliester', 'Buzo', 2, NULL),
(3, 150, '2024-03-10', 'S', 'Verde', 'Algodón', 'Buzo', 3, NULL),
(4, 250, '2024-03-15', 'M', 'Negro', 'Lana', 'Buzo', 4, NULL),
(5, 300, '2024-03-20', 'XL', 'Gris', 'Sintético', 'Buzo', 5, NULL),
(6, 400, '2024-03-25', 'L', 'Blanco', 'Algodón', 'Buzo', 6, NULL),
(7, 350, '2024-04-01', 'M', 'Azul', 'Poliester', 'Buzo', 7, NULL),
(8, 500, '2024-04-05', 'S', 'Rojo', 'Lana', 'Buzo', 8, NULL),
(9, 450, '2024-04-10', 'XL', 'Negro', 'Sintético', 'Buzo', 9, NULL),
(10, 550, '2024-04-15', 'L', 'Verde', 'Algodón', 'Buzo', 10, NULL);
...
CREATE TABLE `usuarios` (
  ...
  `imagen_perfil` varchar(30) DEFAULT NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci;
...
INSERT INTO `usuarios` (`num_doc`, `t_doc`, `usu_nombres`, `usu_apellidos`, `usu_fecha_nacimiento`, `usu_sexo`, `usu_direccion`, `usu_telefono`, `usu_email`, `usu_fecha_contratacion`, `usu_estado`, `rol_id_rol`, `perfil_id_perfil`, `imagen_perfil`) VALUES
(10105885, 1, 'Juan', 'Perez', '1990-01-01', 'M', 'Calle 123', '3001234567', 'juan.perez@example.com', '2024-01-01', 'Activo', 1, 1, NULL),
(10274586, 2, 'Ana', 'Gomez', '1985-02-02', 'F', 'Calle 456', '3002345678', 'ana.gomez@example.com', '2024-02-01', 'Activo', 2, 2, NULL),
(10378547, 3, 'Carlos', 'Lopez', '1975-03-03', 'M', 'Calle 789', '3003456789', 'carlos.lopez@example.com', '2024-03-01', 'Activo', 3, 3, NULL),
(10452456, 4, 'Maria', 'Rodriguez', '1995-04-04', 'F', 'Calle 012', '3004567890', 'maria.rodriguez@example.com', '2024-04-01', 'Activo', 4, 4, NULL),
(10547856, 5, 'Pedro', 'Martinez', '1980-05-05', 'M', 'Calle 345', '3005678901', 'pedro.martinez@example.com', '2024-05-01', 'Activo', 5, 5, NULL),
(10652867, 6, 'Luisa', 'Garcia', '1993-06-06', 'F', 'Calle 678', '3006789012', 'luisa.garcia@example.com', '2024-06-01', 'Activo', 6, 6, NULL),
(10769878, 7, 'Jorge', 'Hernandez', '1988-07-07', 'M', 'Calle 901', '3007890123', 'jorge.hernandez@example.com', '2024-07-01', 'Activo', 7, 7, NULL),
(10785259, 9, 'Rosa', 'Diaz', '1992-09-09', 'F', 'Calle 567', '3009012345', 'rosa.diaz@example.com', '2024-09-01', 'Activo', 9, 9, NULL),
(14578608, 8, 'Laura', 'Ramirez', '1977-08-08', 'F', 'Calle 234', '3008901234', 'laura.ramirez@example.com', '2024-08-01', 'Activo', 8, 8, NULL),
(17865710, 10, 'Luis', 'Torres', '1983-10-10', 'M', 'Calle 890', '3000123456', 'luis.torres@example.com', '2024-10-01', 'Activo', 10, 10, NULL);

--
-- Disparadores `usuarios`
--
DELIMITER $$
CREATE TRIGGER `after_update_proveedor` AFTER UPDATE ON `usuarios` FOR EACH ROW BEGIN
    IF NEW.rol_id_rol = 8 AND OLD.rol_id_rol <> 8 THEN
        UPDATE roles_usuarios
        SET rol_nombre = 'Proveedor'
        WHERE usuario_id = NEW.rol_id_rol;
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_delete_proveedor` BEFORE DELETE ON `usuarios` FOR EACH ROW BEGIN
    IF OLD.rol_id_rol = 8 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No se puede eliminar un usuario con rol de proveedor (rol_id_rol = 8).';
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `before_insert_proveedor` BEFORE INSERT ON `usuarios` FOR EACH ROW BEGIN
    IF NEW.rol_id_rol = 8 THEN
        INSERT INTO auditoria_usuarios (accion, usuario_id, fecha)
        VALUES ('Inserción de proveedor', NEW.rol_id_rol, NOW());
    END IF;
END
$$
DELIMITER ;
